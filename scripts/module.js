...
Hooks.once("init", function () {
  game.handRaiser = new HandRaiser();
  registerSettings();
  ...
  if (game.settings.get("raise-my-hand", "xcard")) {
    game.keybindings.register(moduleName, "xCard", {
      ...
      onDown: () => {
        game.handRaiser.showXCardDialogForEveryoneSocket();
      },
      onUp: () => {},
      restricted: false, // Restrict this Keybinding to gamemaster only?
      reservedModifiers: [],
...
Hooks.on("getSceneControlButtons", function (controls) {
  ...
  tileControls.tools.push({
    ...
    title: "✋Raise My Hand",
    button: true,
    onClick: () => game.handRaiser.toggle(),
  });
  ...
  if (game.settings.get("raise-my-hand", "xcard")) {
    tileControls.tools.push({
      ...
      title: "X Card",
      button: true,
      onClick: () => game.handRaiser.showXCardDialogForEveryoneSocket(),
    });
  }
});
